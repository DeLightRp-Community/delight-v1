...
local function VanRobberyHack()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "VanRobberyHack", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-90",
        firstStreet = locationInfo,
        gender = gender,
        model = nil,
        ...
        dispatchMessage = _U('VanRobberyHack'), -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('VanRobberyHack', VanRobberyHack)
