...
local function ShotFire()
    ...
    TriggerServerEvent("dispatch:server:notify",{
        ...
        dispatchMessage = _U('shotfire'), -- message
        job = {"police"} -- jobs that will get the alerts
    })
end exports('ShotFire', ShotFire)

RegisterNetEvent("ps-dispatch:client:fireshot", function()
    ShotFire()
end)  
...
local function Illegal_Mining()
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify",{
        dispatchcodename = "mining", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-90",
        firstStreet = locationInfo,
        gender = nil,
        model = nil,
        ...
        dispatchMessage = "Illegal Mining", -- message
        job = {"police"} -- jobs that will get the alerts
    })
...
end exports('StoreSafeRobbery', StoreSafeRobbery)
