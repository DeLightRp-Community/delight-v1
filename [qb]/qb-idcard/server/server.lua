...
RegisterNetEvent('qb-idcard:server:showbusinesscard', function(PlayerId)
    local src = source
    local character = QBCore.Functions.GetPlayer(src)
    local item={
        info = {
            firstname = character.PlayerData.charinfo.firstname,
            lastname = character.PlayerData.charinfo.lastname,
            gender = character.PlayerData.charinfo.gender,
            dob = character.PlayerData.job.name,
            nationality = character.PlayerData.charinfo.phone,
            type = "item"
        }
    }
    ShowId(source, item, 'businesscard')
end)
