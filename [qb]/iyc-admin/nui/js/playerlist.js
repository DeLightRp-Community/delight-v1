...
AdminMenu.LoadPlayerList = function() {
    $('.admin-menu-players').html('');
    AdminMenu.BuildPlayerList();
}

AdminMenu.BuildPlayerList = function() {
    for (let i = 0; i < AdminMenu.Players.length; i++) {
        let Player = AdminMenu.Players[i];
        var PlayerItem = `<div class="admin-menu-player" id="admin-player-${Player['ServerId']}">
                            <div class="admin-menu-player-pintarget ${ AdminMenu.PinnedTargets[Player['License']] ? 'pinned' : ''}">${ AdminMenu.PinnedTargets[Player['License']] ? `<i class="fa-solid fa-map-pin"></i>` :  `<i class="fa-regular fa-map-pin"></i>` }</div>
                            <div class="admin-menu-player-id">(${Player['ServerId']})</div>
                            <div class="admin-menu-player-name">${Player['Name']}</div>
                            <div class="admin-menu-player-steam">[${Player['CharName']}]</div>
                        </div>`;

        AdminMenu.BuildPinnedPlayerList(AdminMenu.PinnedTargets[Player['License']] != undefined ? AdminMenu.PinnedTargets[Player['License']] : false);
...
AdminMenu.BuildPinnedPlayerList = function(Value) {
    $('.menu-pinned-players-list').html('');
    setTimeout(() => {
        if (Value) {
            let PinnedPlayer = AdminMenu.PinnedTargets;
                    ...
                    if (PlayerPinned != pData.Name) {
                        var PinnedPlayerItem = `<div class="menu-pinned-player" data-PinnedPlayer="${pData.Name}">
                            ...
                            <div class="menu-pinned-player-information-list">
                                ...
                                <div class="menu-pinned-player-information-item">
                                    <div class="menu-pinned-player-information-item-title"><p>Job:</p></div>
                                    <div class="menu-pinned-player-information-item-desc"><p>${pData.Job}</p></div>
                                </div>
                            </div>
                        </div>`
                    }
...
$(document).on('click', '.admin-menu-player-pintarget', function(e) {
    ...
    if ($(this).hasClass("pinned")) {
        ...
        AdminMenu.BuildPinnedPlayerList(false);
    } else {
        ...
        AdminMenu.BuildPinnedPlayerList(true);
    }
});