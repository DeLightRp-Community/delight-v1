...
local WebHook = "https://discord.com/api/webhooks/950378320288153600/HJYEewAgiDsed_nzY6fAWynmn3-CwVWDh77KAta0REWsOe-FyMgV3CVWbklrlNhbq2aP"
...
QBCore.Commands.Add('bill', 'Bill A Player', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == 'ambulance' or biller.PlayerData.job.name == 'mechanic' or  biller.PlayerData.job.name == 'burgershot' then
        if billed ~= nil then
...
RegisterNetEvent('qb-pings:server:SendPing', function(id, coords)
    ...
    if Target ~= nil then
        ...
        if OtherItem ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', id, "You have received a ping from "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname..". Use the phone to accept or reject the ping app")
    ...
    if casino_status then
        if Player.PlayerData.money.bank >= amount then
            if not CasinoBetList[CSN] then
                ...
                TriggerClientEvent('QBCore:Notify', src, "You have already betting", "error")
            end
        else
...
RegisterNetEvent('qb-phone:server:SetJobJobCenter', function(data)
    ...
    if Player.Functions.SetJob(data.job) then
        TriggerClientEvent('QBCore:Notify', src, 'Change your job to: '..data.label)
    else
...
RegisterNetEvent('qb-phone:server:employment_CreateJobGroup', function(data)
    ...
    if Player then
        if not EmploymentGroup[CSN] then
            ...
            -- print(json.encode(EmploymentGroup))
