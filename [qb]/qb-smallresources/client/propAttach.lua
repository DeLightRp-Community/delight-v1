...
AddEventHandler('attachPropPerm', function(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	APPyR = yR
	APPzR = zR
	exports['qb-core']:DrawText('press 7 to drop the bag', 'left')
...
AddEventHandler("fivemedical:dropmedkit", function()
	...
	removeAttachedPropPerm()
			...
			droppedmedkit = CreateObject("xm_prop_x17_bag_med_01a", x + 0.5, y, z - 1 , true, true, true)		
			randPickupAnim()
			RequestModel(droppedmedkit)
			while not HasModelLoaded(droppedmedkit) do
				Citizen.Wait(100)
			end
...
RegisterNetEvent('pickup:bag', function()
	local x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(-1), true))
	local droppedmedkit = GetClosestObjectOfType (x + 0.5, y, z - 1, 10.0, GetHashKey("xm_prop_x17_bag_med_01a"),true,false,false)
	if DoesEntityExist(droppedmedkit) then
		DeleteObject(droppedmedkit)
	else
		return
	end
	TriggerEvent( 'attach:medicalBag' )
...
exports["qb-target"]:AddTargetModel(bag, {
	options = {
		{
			type = "client",
			event = "pickup:bag",
