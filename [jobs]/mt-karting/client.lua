...
RegisterNetEvent('mt-karting:client:hasTicket', function()
    local time = Config.Tickets[4].time * 60

    if TicketAtivo == false then
        Time = time
        TicketAtivo = true
        SpawnKart(Config.Tickets[4].time * 60)
    end
end)
...
RegisterNetEvent('mt-karting:client:Ticket', function(args)
   -- local EliminarVeiculo = QBCore.Functions.GetClosestVehicle()
    ...
    elseif args == 4 and TicketAtivo == false then
       ...
       -- startTimer(Time)
        SpawnKart(Time)
    elseif args == 10 and TicketAtivo == false then
        -- local time4 = Config.Tickets[4].time * 60

        TriggerServerEvent('mt-karting:server:checkTicket')
        -- Time = time4
        -- TicketAtivo = true
       -- startTimer(Time)
        -- SpawnKart(Time)
