...
CreateThread(function()
	while true do
		...
		if fishing then
			...
			QBCore.Functions.Notify('Press [F] to Catch Fish!', 'error', 2000)
			input = 0
			pausetimer = 0
		end
	end
...
RegisterNetEvent('fishing:fishstart', function()
	...
	if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z - 3.0)  then
		...
		Wait(time)
		QBCore.Functions.Notify('Press [X] to stop fishing at any time', 'primary', 5000)
...
loseBaitAnimation = function()
	...
	QBCore.Functions.Notify('Fish took your bait!', 'primary', 3000)
	Wait(2000)
