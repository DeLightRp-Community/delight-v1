...
RegisterCommand("mlist", function(source, args)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)

    if xPlayer.PlayerData.job.name == "ambulance" or xPlayer.PlayerData.job.name == "police" then
        local type = "toggle"
        TriggerEvent("nv:medic:refresh")
        
        if args[1] == "0" then
            type = "drag"
        end

        TriggerClientEvent("nv:medic:open", src, type)
    end
end)
...
RegisterServerEvent("nv:medic:refresh")
AddEventHandler("nv:medic:refresh", function()
    local new = ""

    for k,v in pairs(QBCore.Functions.GetPlayers()) do
        local xPlayer = QBCore.Functions.GetPlayer(v)
        if xPlayer and (xPlayer.PlayerData.job.name == 'ambulance') then
            ...
            if radioChannel == 0 or radioChannel >10 then
                radioLabel = "Off"
            else
    ...
    HTML = new
    TriggerClientEvent("nv:medic:refresh", -1, HTML)
end)
