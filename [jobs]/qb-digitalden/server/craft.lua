...
RegisterNetEvent('inventory:server:SaveInventory', function(type, id)
    local src = source
    -- print(id)
    if type=="stash" and id~=nil and id =="digitaldencraft" then
        local result ={}
        Wait(100)
        ...
        if result[1]~=nil and itemOne~=nil and itemOne.name == "recyclepart" then
            ...
            TriggerClientEvent('qb-digitalden:carf:choose',src,itemOne.amount)
        else
...
QBCore.Functions.CreateCallback('qb-digitalden:server:craftiLiggalItem', function (source, cb, data)
    local src = source
...
QBCore.Functions.CreateCallback('qb-digitalden:server:craftItem', function (source, cb, data)
    local src = source
    -- print(QBCore.Shared.Items[data.name].weight)
    -- ''
    TriggerClientEvent('qb-digitalden:craft:disable',-1,data.count)
    ...
    MySQL.Sync.fetchAll('UPDATE stashitems SET items=@item WHERE stash=@stash', { ["@item"] = item, ["@stash"] = 'digitaldencraft' })
    return cb(true)
