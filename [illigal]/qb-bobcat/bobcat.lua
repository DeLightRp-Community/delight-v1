...
function HackSuccess()
    ...
    if Config.SpawnGuards then 
        TriggerEvent('gc-bobcatheist:client:npcSync') 
        --print("Peds have been spawned!")
    end
    ClearPedTasks(ped)
    TriggerServerEvent('gc-bobcatheist:server:vaultsync')
end
...
RegisterNetEvent('gc-bobcatheist:client:vaultsync', function()
    killedpeds = true
end)

local guards = {}

RegisterNetEvent('gc-bobcatheist:client:npcSync', function()
    SpawnGuards()
end) 
