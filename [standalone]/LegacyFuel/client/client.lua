...
CreateThread(function()
	...
	exports['qb-target']:AddTargetBone(bones, {
		...
		{
			type = "client",
			event = "LegacyFuel:client:SendMenuToServer",
...
RegisterNetEvent('LegacyFuel:client:buyCanMenu', function()
    exports['qb-menu']:openMenu({
        {
            header = "Gas Station",
            txt = 'The total cost is going to be: $'..Config.canCost..' including taxes.',
            params = {
                event = "LegacyFuel:client:buyCan",
            }
        },
    })
end)

RegisterNetEvent('LegacyFuel:client:buyCan', function()
	local ped = PlayerPedId()
    if not HasPedGotWeapon(ped, 883325847) then
		if QBCore.Functions.GetPlayerData().money['cash'] >= Config.canCost then
			...
			TriggerServerEvent('LegacyFuel:server:PayForFuel', Config.canCost, GetPlayerServerId(PlayerId()))
		else
...
RegisterNetEvent('LegacyFuel:client:refuelCanMenu', function()
	local ped = PlayerPedId()
	local price = 0
		...
		exports['qb-menu']:openMenu({
			{
				...
				params = {
					event = "LegacyFuel:client:refuelCan",
				}
			},
		})
	else
...
RegisterNetEvent('LegacyFuel:client:refuelCan', function()
	...
	if GetAmmoInPedWeapon(ped, 883325847) < 4500 then
		...
		while not HasAnimDictLoaded('weapon@w_sp_jerrycan') do Wait(100) end
			...
			TriggerServerEvent('LegacyFuel:server:PayForFuel', Config.refuelCost, GetPlayerServerId(PlayerId()))
...
RegisterNetEvent('LegacyFuel:client:SendMenuToServer', function()
	local vehicle = QBCore.Functions.GetClosestVehicle()
	local CurFuel = GetVehicleFuelLevel(vehicle)
	...
	if HasPedGotWeapon(ped, 883325847) then
		if GetAmmoInPedWeapon(ped, 883325847) ~= 0 then
			if CurFuel < 95 then
				TriggerServerEvent('LegacyFuel:server:OpenMenu', 0, inGasStatio, true)
			else
		...
		if CurFuel < 95 then
			TriggerServerEvent('LegacyFuel:server:OpenMenu', refillCost, inGasStation, false)
		else
			QBCore.Functions.Notify(Lang:t("notify.vehicle_full"), "error")
		end
	end
...
RegisterNetEvent('LegacyFuel:client:RefuelVehicle', function(refillCost)
	local ped = PlayerPedId()
	local vehicle = QBCore.Functions.GetClosestVehicle()
	local ped = PlayerPedId()
	...
	if HasPedGotWeapon(ped, 883325847) then
		...
		if ammo <= 40 then
			...
			TriggerEvent("LegacyFuel:client:fuelTick", vehicle)
		...
		if inGasStation then
			if isCloseVeh() then
				...
				QBCore.Functions.Progressbar("refuel-car", "Refueling", time, false, true, {
					...
					TriggerServerEvent('LegacyFuel:server:PayForFuel', refillCost, GetPlayerServerId(PlayerId()))
					SetFuel(vehicle, 100)
					PlaySound(-1, "5_SEC_WARNING", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
					StopAnimTask(ped, "weapon@w_sp_jerrycan", "fire", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
				end, function() -- Cancel
...
exports['qb-target']:AddTargetModel(props, {
	options = {
		{
			type = "client",
			event = "LegacyFuel:client:buyCanMenu",
			icon = "fas fa-burn",
			label = "Buy Jerry Can",
		...
		{
			type = "client",
			event = "LegacyFuel:client:refuelCanMenu",
			icon = "fas fa-gas-pump",
